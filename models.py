...
class Nut:
    def __init__(self, ma_so, toa_do_x, toa_do_y):
        ...
        self.danh_sach_ke = []          # Danh sach hang xom: [(nut, trong_so)...]
...
def tao_do_thi_ngau_nhien(co_huong=False):
    ...
    while chua_ket_noi:
        ...
        if len(ung_vien) > 8: ung_vien = random.sample(ung_vien, 8)

        for hien_tai in ung_vien:
            for muc_tieu in chua_ket_noi:
                if (hien_tai.ma_so == 0 and muc_tieu.ma_so == so_luong-1) or (hien_tai.ma_so == so_luong-1 and muc_tieu.ma_so == 0): continue
                if len(hien_tai.danh_sach_ke) >= 3: continue 
                
                d = tinh_khoang_cach(hien_tai, muc_tieu)
                if d < min_dist:
                    min_dist = d; u = hien_tai; v = muc_tieu
    ...
    for u in danh_sach_nut:
        cac_nut_khac = sorted(danh_sach_nut, key=lambda x: tinh_khoang_cach(u, x))
        gan_nhat = cac_nut_khac[1:6] 
        random.shuffle(gan_nhat)
        
        for v in gan_nhat:
            if u == v or (u.ma_so==0 and v.ma_so==so_luong-1) or (u.ma_so==so_luong-1 and v.ma_so==0): continue
...
def khoi_phuc_do_thi(data_luu, ds_hien_tai=None):
    ...
    pos_map = {}
    if ds_hien_tai:
        for n in ds_hien_tai: pos_map[n.ma_so] = (n.x, n.y)
        
    new_nodes = []
    # Tao nut moi
    for d in data_luu:
        x, y = d['x'], d['y']
        if d['id'] in pos_map: x, y = pos_map[d['id']]
...
def chuyen_thanh_co_huong(danh_sach_nut):
    ...
    for u in danh_sach_nut:
        ...
        for v, w in danh_sach_copy:
            ...
            if cap_canh in da_xu_ly: continue
            
            v_noi_u = False
            for k, kw in v.danh_sach_ke:
                if k == u: v_noi_u = True; break
            
            if v_noi_u:
                if (u.ma_so + v.ma_so) % 2 == 0: 
                    danh_sach_moi = []
                    for k, kw in v.danh_sach_ke:
                        if k != u: danh_sach_moi.append((k, kw))
...
def phan_tich_van_ban(van_ban):
    ...
    if not cac_dong: return None
    
    try:
        so_luong = int(cac_dong[0])
        for i in range(so_luong): danh_sach_nut.append(Nut(i, 0, 0))
        sap_xep_vi_tri_tren_luoi(danh_sach_nut, so_luong)
        ...
        for l in cac_dong[1:]:
            ...
            if len(mang_so) >= 3:
                ...
                if 0 <= u < so_luong and 0 <= v < so_luong:
                    danh_sach_nut[u].them_canh(danh_sach_nut[v], w, hai_chieu=False)
        return danh_sach_nut
    except:
        return None
...
def luu_lich_su_nhap(van_ban):
    ...
    cu = lay_lich_su_nhap()
    if cu and cu[0].strip() == van_ban.strip(): return 
...
def doc_file_day_du(ten_file):
    ...
    try:
        ...
        dong_che_do = -1; dong_so_nut = -1; dong_canh = -1; dong_ket_thuc = -1
        
        for i, l in enumerate(cac_dong):
            if l.startswith("CHE_DO:"): dong_che_do = i
            elif l.startswith("SO_NUT:"): dong_so_nut = i
            elif l.startswith("CANH:"): dong_canh = i
            elif l.startswith("KET_THUC_DU_LIEU"): dong_ket_thuc = i
            
        if dong_che_do == -1 or dong_so_nut == -1: return None, True
        ...
        for i in range(dong_canh + 1, diem_dung):
            if not cac_dong[i]: continue
            phan = list(map(int, cac_dong[i].split()))
            if len(phan) >= 3:
                u_id, v_id, w = phan[0], phan[1], phan[2]
                u_obj = next((n for n in danh_sach_nut if n.ma_so == u_id), None)
                v_obj = next((n for n in danh_sach_nut if n.ma_so == v_id), None)
                if u_obj and v_obj:
