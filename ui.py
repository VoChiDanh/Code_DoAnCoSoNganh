...
def cat_dong_van_ban(van_ban, font, chieu_rong_toi_da):
    ...
    for tu in cac_tu:
        ...
        if font.size(thu)[0] < chieu_rong_toi_da:
            ...
            if dong_hien_tai: cac_dong.append(dong_hien_tai)
            dong_hien_tai = tu + " "
    if dong_hien_tai: cac_dong.append(dong_hien_tai)
    return cac_dong
...
def ve_duong_noi(man_hinh, diem_dau, diem_cuoi, mau=MAU_DEN, do_day=2, ti_le=1.0):
    dx = diem_cuoi[0] - diem_dau[0]; dy = diem_cuoi[1] - diem_dau[1]
    d = math.hypot(dx, dy)
    if d == 0: return diem_dau
    
    if d < 180 * ti_le: 
        rut_ngan = 6 * ti_le
        px, py = -dy/d, dx/d
        sx = diem_dau[0] + px*rut_ngan; sy = diem_dau[1] + py*rut_ngan
        ex = diem_cuoi[0] + px*rut_ngan; ey = diem_cuoi[1] + py*rut_ngan
        ...
        pygame.draw.line(man_hinh, mau, (sx, sy), (dich_thuc, dich_thuc_y), do_day)
        ...
        pygame.draw.polygon(man_hinh, mau, [(dich_thuc, dich_thuc_y), (x1, y1), (x2, y2)])
        ...
        cac_diem = [duong_cong_bezier(diem_dau, (cx, cy), diem_cuoi, t/20) for t in range(21)]
        if len(cac_diem) > 1: pygame.draw.lines(man_hinh, mau, False, cac_diem, do_day)
...
def lay_mau_nut(nut, nut_dang_chon, duong_di):
    ...
    if mau_nen in [MAU_DEN, MAU_XANH_DUONG]: mau_chu = MAU_TRANG
    mo_di = False
    
    if nut_dang_chon:
        if nut == nut_dang_chon: mau_nen, mau_vien, mau_chu = MAU_TRANG, MAU_DEN, MAU_DEN
        else:
            di_ra = any(k==nut_dang_chon for k, w in nut.danh_sach_ke)
            di_vao = any(k==nut for k, w in nut_dang_chon.danh_sach_ke)
            if di_ra: mau_nen, mau_vien, mau_chu = MAU_DO, MAU_DO, MAU_TRANG
            elif di_vao: mau_nen, mau_vien, mau_chu = MAU_XANH_LA, MAU_XANH_LA, MAU_TRANG
            else: mau_nen, mau_vien, mau_chu = (240,240,240), (220,220,220), (200,200,200); mo_di = True
            
    if nut in duong_di and not nut_dang_chon:
        mau_vien = MAU_DO
        mau_chu = MAU_TRANG if mau_nen in [MAU_DEN, MAU_XANH_DUONG] else MAU_DEN
...
class NutBam:
    def __init__(self, x, y, rong, cao, van_ban, mau=MAU_XANH_LA):
        ...
        self.noi_dung = van_ban; self.mau = mau
        self.font = pygame.font.SysFont('Arial', 18, bold=True)
        self.gia_tri_an = "" 

    def ve(self, man_hinh):
        bong = self.hinh_chu_nhat.copy(); bong.x += 2; bong.y += 2
        pygame.draw.rect(man_hinh, (200, 200, 200), bong, border_radius=8)
        pygame.draw.rect(man_hinh, self.mau, self.hinh_chu_nhat, border_radius=8)
        pygame.draw.rect(man_hinh, MAU_DEN, self.hinh_chu_nhat, 2, border_radius=8) 
        text_surf = self.font.render(self.noi_dung, True, MAU_TRANG)
        man_hinh.blit(text_surf, text_surf.get_rect(center=self.hinh_chu_nhat.center))

    def duoc_nhan(self, vi_tri_chuot): return self.hinh_chu_nhat.collidepoint(vi_tri_chuot)
...
class HopNhapLieu:
    ...
    def xoa_vung_chon(self):
        if self.con_tro == self.diem_neo: return False
        dau, cuoi = sorted([self.con_tro, self.diem_neo])
        r1, c1 = dau; r2, c2 = cuoi
        ...
        if r1 == r2:
            ...
            for _ in range(r2-r1): self.cac_dong.pop(r1+1)
        
        self.con_tro = [r1, c1]; self.diem_neo = [r1, c1]
        return True
    ...
    def chen_chu(self, chu):
        ...
        danh_sach = chu.split('\n')
        r, c = self.con_tro
        dau = self.cac_dong[r][:c]; cuoi = self.cac_dong[r][c:]
        ...
        if len(danh_sach) == 1:
            ...
            for i in range(1, len(danh_sach)-1): self.cac_dong.insert(r+i, danh_sach[i])
            self.cac_dong.insert(r+len(danh_sach)-1, danh_sach[-1] + cuoi)
            self.con_tro = [r+len(danh_sach)-1, len(danh_sach[-1])]
        self.diem_neo = list(self.con_tro)

    def copy(self):
        if self.con_tro == self.diem_neo: return
        dau, cuoi = sorted([self.con_tro, self.diem_neo])
        r1, c1 = dau; r2, c2 = cuoi
        t = ""
        if r1 == r2: t = self.cac_dong[r1][c1:c2]
        else:
            t = self.cac_dong[r1][c1:] + "\n"
            for i in range(r1+1, r2): t += self.cac_dong[i] + "\n"
        ...
        if e.type == pygame.MOUSEBUTTONDOWN:
            if self.hinh_chu_nhat.collidepoint(e.pos):
                ...
                if e.button == 1:
                    ...
                    self.con_tro = t
                    if not shift: self.diem_neo = list(t)
                    self.dang_keo_chuot = True
                elif e.button == 4: self.cuon_doc = max(0, self.cuon_doc - 1)
                elif e.button == 5: self.cuon_doc += 1
            else: self.dang_kich_hoat = False

        elif e.type == pygame.MOUSEBUTTONUP:
            if e.button == 1: self.dang_keo_chuot = False
            ...
            if self.dang_kich_hoat and self.dang_keo_chuot:
                ...
                if self.con_tro[0] < self.cuon_doc: self.cuon_doc = self.con_tro[0]
                if self.con_tro[0] > self.cuon_doc + so_dong_hien_thi - 2: self.cuon_doc = self.con_tro[0] - so_dong_hien_thi + 2

        elif e.type == pygame.KEYDOWN and self.dang_kich_hoat:
            da_di_chuyen = False
            if e.key == pygame.K_UP:
                if self.con_tro[0] > 0: self.con_tro[0]-=1; self.con_tro[1]=min(self.con_tro[1], len(self.cac_dong[self.con_tro[0]]))
                da_di_chuyen = True
            elif e.key == pygame.K_DOWN:
                if self.con_tro[0] < len(self.cac_dong)-1: self.con_tro[0]+=1; self.con_tro[1]=min(self.con_tro[1], len(self.cac_dong[self.con_tro[0]]))
                da_di_chuyen = True
            elif e.key == pygame.K_LEFT:
                if self.con_tro[1]>0: self.con_tro[1]-=1
                elif self.con_tro[0]>0: self.con_tro[0]-=1; self.con_tro[1]=len(self.cac_dong[self.con_tro[0]])
                da_di_chuyen = True
            elif e.key == pygame.K_RIGHT:
                if self.con_tro[1]<len(self.cac_dong[self.con_tro[0]]): self.con_tro[1]+=1
                elif self.con_tro[0]<len(self.cac_dong)-1: self.con_tro[0]+=1; self.con_tro[1]=0
                da_di_chuyen = True
            
            if da_di_chuyen and not shift: self.diem_neo = list(self.con_tro)

            if ctrl and e.key == pygame.K_a: self.diem_neo=[0,0]; self.con_tro=[len(self.cac_dong)-1, len(self.cac_dong[-1])]
            elif ctrl and e.key == pygame.K_c: self.copy()
            elif ctrl and e.key == pygame.K_v: self.paste()
            elif ctrl and e.key == pygame.K_x: self.copy(); self.xoa_vung_chon()
            
            elif e.key == pygame.K_RETURN: self.chen_chu("\n")
            elif e.key == pygame.K_BACKSPACE:
                if self.con_tro != self.diem_neo: self.xoa_vung_chon()
                else:
                    r, c = self.con_tro
                    if c>0: self.cac_dong[r] = self.cac_dong[r][:c-1]+self.cac_dong[r][c:]; self.con_tro[1]-=1
                    elif r>0:
                        pr = self.cac_dong[r-1]; cr = self.cac_dong[r]
                        self.cac_dong[r-1] = pr+cr; self.cac_dong.pop(r); self.con_tro=[r-1, len(pr)]
                    self.diem_neo = list(self.con_tro)
            elif e.key == pygame.K_DELETE:
                if self.con_tro != self.diem_neo: self.xoa_vung_chon()
                else:
                    r, c = self.con_tro
                    if c<len(self.cac_dong[r]): self.cac_dong[r] = self.cac_dong[r][:c]+self.cac_dong[r][c+1:]
                    elif r<len(self.cac_dong)-1: self.cac_dong[r]+=self.cac_dong[r+1]; self.cac_dong.pop(r+1)
            elif e.unicode and e.unicode.isprintable() and not ctrl: self.chen_chu(e.unicode)
    ...
    def ve(self, man_hinh):
        ...
        so_dong_hien_thi = self.hinh_chu_nhat.height // self.chieu_cao_dong
        if self.cuon_doc > len(self.cac_dong) - so_dong_hien_thi: self.cuon_doc = max(0, len(self.cac_dong) - so_dong_hien_thi)

        man_hinh.set_clip(self.hinh_chu_nhat.inflate(-10, -10))
        sx = self.hinh_chu_nhat.x + 5; sy = self.hinh_chu_nhat.y + 5
        ...
        for i in range(self.cuon_doc, min(len(self.cac_dong), self.cuon_doc + so_dong_hien_thi + 1)):
            ...
            if co_chon:
                if dau[0] < i < cuoi[0]:
                    ...
                    pygame.draw.rect(man_hinh, (180, 210, 255), (sx, y, w+5, self.chieu_cao_dong))
                elif i == dau[0] and i == cuoi[0]:
                    x1, _ = self.font.size(dong[:dau[1]]); x2, _ = self.font.size(dong[:cuoi[1]])
                    pygame.draw.rect(man_hinh, (180, 210, 255), (sx+x1, y, x2-x1, self.chieu_cao_dong))
                elif i == dau[0]:
                    x1, _ = self.font.size(dong[:dau[1]]); wf, _ = self.font.size(dong)
                    pygame.draw.rect(man_hinh, (180, 210, 255), (sx+x1, y, wf-x1+5, self.chieu_cao_dong))
                elif i == cuoi[0]:
                    x2, _ = self.font.size(dong[:cuoi[1]])
                    pygame.draw.rect(man_hinh, (180, 210, 255), (sx, y, x2, self.chieu_cao_dong))
            ...
            if self.dang_kich_hoat and i == self.con_tro[0]:
                if pygame.time.get_ticks() % 1000 < 500:
                    ...
                    pygame.draw.line(man_hinh, MAU_DEN, (sx+cx, y), (sx+cx, y+self.chieu_cao_dong), 2)
...
def ve_toan_bo_do_thi(man_hinh, danh_sach_nut, font, co_huong=False, duong_di=[], nut_dang_chon=None, ti_le=1.0, do_lech=(0,0)):
    for n in danh_sach_nut:
        ...
        for ke, w in n.danh_sach_ke:
            ...
            mau = MAU_DEN; do_day = max(1, int(2*ti_le))
            
            if nut_dang_chon:
                mau = (230, 230, 230); do_day = max(1, int(1*ti_le))
                if n == nut_dang_chon: mau = MAU_XANH_LA; do_day = max(2, int(4*ti_le))
                elif ke == nut_dang_chon: mau = MAU_DO; do_day = max(2, int(4*ti_le))
            elif len(duong_di) > 1:
                for i in range(len(duong_di)-1):
                    if (duong_di[i]==n and duong_di[i+1]==ke):
                        mau = MAU_DO; do_day = max(2, int(5*ti_le)); break
                    if not co_huong and (duong_di[i]==ke and duong_di[i+1]==n):
                        mau = MAU_DO; do_day = max(2, int(5*ti_le)); break

            if co_huong: ve_duong_noi(man_hinh, (nx, ny), (kx, ky), mau, do_day, ti_le)
            else: 
                if n.ma_so < ke.ma_so:
    ...
    for n in danh_sach_nut:
        nx, ny = chuyen_toa_do_man_hinh(n.x, n.y, ti_le, do_lech)
        for ke, w in n.danh_sach_ke:
            if not co_huong and n.ma_so > ke.ma_so: continue
            kx, ky = chuyen_toa_do_man_hinh(ke.x, ke.y, ti_le, do_lech)
            hien = True; mau = MAU_DEN
            if nut_dang_chon:
                hien = False
                if n == nut_dang_chon or ke == nut_dang_chon: 
                    mau = MAU_XANH_LA if n == nut_dang_chon else MAU_DO; hien = True
            elif len(duong_di) > 1:
                for i in range(len(duong_di)-1):
                    if (duong_di[i]==n and duong_di[i+1]==ke):
                        mau = MAU_DO; hien = True; break
                    if not co_huong and (duong_di[i]==ke and duong_di[i+1]==n):
                        mau = MAU_DO; hien = True; break
            
            if hien:
                pos = ve_duong_noi(man_hinh, (nx, ny), (kx, ky), (0,0,0), 0, ti_le) if co_huong else lay_trung_diem((nx, ny), (kx, ky))
                if pos:
                    pygame.draw.circle(man_hinh, MAU_TRANG, (int(pos[0]), int(pos[1])), int(10*ti_le))
                    if mau == MAU_DEN: mau = MAU_XANH_LA
                    t = font.render(str(w), True, mau)
                    man_hinh.blit(t, t.get_rect(center=(int(pos[0]), int(pos[1]))))

    W, H = man_hinh.get_size()
    r = max(5, int(BAN_KINH_NUT * ti_le))
    for n in danh_sach_nut:
        sx, sy = chuyen_toa_do_man_hinh(n.x, n.y, ti_le, do_lech)
        if not (-r < sx < W+r and -r < sy < H+r): continue
        
        mau_nen, mau_vien, mau_chu, mo_di = lay_mau_nut(n, nut_dang_chon, duong_di)
        pygame.draw.circle(man_hinh, mau_nen, (int(sx), int(sy)), r)
        pygame.draw.circle(man_hinh, mau_vien, (int(sx), int(sy)), r, max(1, int(2*ti_le)))
...
def ve_bang_ket_qua(man_hinh, lich_su, cuon_doc):
    W, H = man_hinh.get_size(); rong_bang = 450; x = W - rong_bang
    ...
    y = 35 + cuon_doc; tong_chieu_cao = 0
    
    for hang in lich_su:
        h = 0
        if isinstance(hang, str) and (hang.startswith("KET QUA") or hang.startswith(">>")):
            ls = cat_dong_van_ban(hang, font_kq, rong_bang-10); h = len(ls)*20 + 10
            if y+h > 0 and y < H:
                pygame.draw.rect(man_hinh, MAU_NEN_KET_QUA, (x, y, rong_bang, h))
                pygame.draw.line(man_hinh, MAU_DO, (x, y), (x+rong_bang, y))
                for i, l in enumerate(ls): man_hinh.blit(font_kq.render(l, True, MAU_DO), (x+10, y+5+i*20))
        else:
            cac_o_con = []
            ...
            for i, gia_tri in enumerate(du_lieu_hien_thi):
                if i < len(cac_cot):
                    ...
                    noi_dung_con = cat_dong_van_ban(str(gia_tri), font_dong, rong_cot - 5)
                    cac_o_con.append(noi_dung_con)
                else: cac_o_con.append([])
            
            h = max(1, max([len(c) for c in cac_o_con])) * 16 + 10
            
            if y+h > 0 and y < H:
                if int(hang[0])%2==0: pygame.draw.rect(man_hinh, MAU_HANG_CHAN, (x, y, rong_bang, h))
                pygame.draw.line(man_hinh, (200,200,200), (x, y+h), (x+rong_bang, y+h))
                cx = x + 5
                for i, cac_dong in enumerate(cac_o_con):
                    mau_chu = MAU_DO if i==1 else (MAU_XANH_DUONG if i==2 else MAU_DEN)
                    for j, l in enumerate(cac_dong): man_hinh.blit(font_dong.render(l, True, mau_chu), (cx, y+5+j*16))
                    cx += cac_cot[i][1]
        y += h; tong_chieu_cao += h
    man_hinh.set_clip(None)
    
    pygame.draw.line(man_hinh, MAU_VIEN_BANG, (x, 0), (x, H), 3)
    pygame.draw.rect(man_hinh, MAU_TIEU_DE_BANG, (x, 0, rong_bang, 35))
    cx = x + 5
    for t, r in cac_cot: man_hinh.blit(pygame.font.SysFont('Arial', 14, bold=True).render(t, True, MAU_TRANG), (cx, 8)); cx += r
...
def hien_cua_so_so_sanh(man_hinh, kq_bfs, kq_dfs, kq_dijk):
    ...
    cac_dong = [
        ...
        ("So buoc chay", str(kq_bfs['buoc']), str(kq_dfs['buoc']), str(kq_dijk['buoc'])),
        ("Tong Chi phi", str(kq_bfs['chi_phi']), str(kq_dfs['chi_phi']), str(kq_dijk['chi_phi'])),
        ("So dinh di qua", str(len(kq_bfs['duong_di'].split('->')) if '->' in kq_bfs['duong_di'] else 0), 
                           str(len(kq_dfs['duong_di'].split('->')) if '->' in kq_dfs['duong_di'] else 0),
