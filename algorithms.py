...
def lay_ket_qua_duong_di(danh_sach_nut):
    ...
    chi_phi = "Khong co"
    if dich.khoang_cach != float('inf'):
        chi_phi = dich.khoang_cach
...
def chay_bfs(nut_bat_dau):
    ...
    while hang_doi:
        ...
        if nut_hien_tai.ma_so in tap_mo:
            tap_mo.remove(nut_hien_tai.ma_so)
        ...
        chuoi_hang_doi = "Q: [" + \
            ", ".join([str(x.ma_so) for x in hang_doi]) + "]"
...
def chay_dfs(nut_bat_dau):
    ...
    while ngan_xep:
        ...
        chuoi_stack = "Stack: [" + \
            ", ".join([str(x.ma_so) for x in ngan_xep]) + "]"
        chuoi_tap_dong = in_danh_sach(thu_tu_duyet)
...
def chay_dijkstra(nut_bat_dau):
    ...
    while hang_doi_uu_tien:
        ...
        if nut_hien_tai.ma_so in chi_phi_tot_nhat_trong_mo and khoang_cach > chi_phi_tot_nhat_trong_mo[nut_hien_tai.ma_so]:
            continue
        tap_dong.add(nut_hien_tai.ma_so)
        if nut_hien_tai.ma_so in chi_phi_tot_nhat_trong_mo:
            del chi_phi_tot_nhat_trong_mo[nut_hien_tai.ma_so]
        ...
        for ke, trong_so in nut_hien_tai.danh_sach_ke:
            ...
            if can_them_vao:
                if khoang_cach_moi < ke.khoang_cach:
                    ...
                    ke.nut_cha = nut_hien_tai
                    ke.mau_sac = MAU_VANG
                heapq.heappush(hang_doi_uu_tien,
                               (ke.khoang_cach, ke.ma_so, ke))
        ...
        xem_truoc = [
            f"N{item[2].ma_so}:{item[0]}" for item in heap_de_hien_thi]
        # Join co dau cach de UI cat dong
        chuoi_heap = f"Heap({len(hang_doi_uu_tien)}): [" + \
            ", ".join(xem_truoc) + "]"
